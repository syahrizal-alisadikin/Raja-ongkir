...
export default {
  setup(){
    ...
    // State ID for provinsi and city
    const state = reactive({
      province_origin:"",
      city_origin:"",

      province_destinantion:"",
      city_destinantion:"",
      weight:""
    })
    ...
    const resultCost = ref(null)
      onMounted(() => {
        ...
        axios.get('http://127.0.0.1:8000/api/provinces').then(response => {
      ...
      function getCitiesOrigin() {
        ...
        axios.get(`http://127.0.0.1:8000/api/cities/${state.province_origin}`).then(response => {
      ...
      function getCostOngkir() {
       ...
       return {
        provinsi, state, cities_origin, cities_destination, getCitiesOrigin, getCitiesDestination, getCostOngkir, resultCost
      }
  }
